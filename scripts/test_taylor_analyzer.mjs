...
import { analyzeTaylorSerial } from "../web/analyzer.js";

const json = async (path) => JSON.parse(await readFile(new URL(path, import.meta.url), "utf8"));

const [taylor, registry, fixtures, schema] = await Promise.all([
  json("../data/brands/taylor/rules.json"),
  json("../data/source-registry.json"),
  json("../data/fixtures/taylor_serial_rules.json"),
...
for (const testCase of fixtures.cases) {
  const result = analyzeTaylorSerial(testCase.input.serial, taylor, registry);
...
console.log("test_taylor_analyzer: ok");
