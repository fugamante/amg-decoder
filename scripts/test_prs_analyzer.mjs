...
import { analyzePrsSerial } from "../web/analyzer.js";

const json = async (path) => JSON.parse(await readFile(new URL(path, import.meta.url), "utf8"));

const [prs, registry, fixtures, schema] = await Promise.all([
  json("../data/brands/prs/rules.json"),
  json("../data/source-registry.json"),
  json("../data/fixtures/prs_set_neck_rules.json"),
...
for (const testCase of fixtures.cases) {
  const result = analyzePrsSerial(testCase.input.serial, prs, registry);
...
console.log("test_prs_analyzer: ok");
